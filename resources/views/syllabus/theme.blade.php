        ...
        @foreach($chunks as $chunk)
            <div class="flex justify-center gap-4 mb-4">
                @foreach($chunk as $index => $theme)
                    <div class="p-6 w-full max-w-xs bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 flex items-center justify-center text-center h-48">
                        <a href="{{ route('syllabus.theme', ['slug' => $slug, 'theme' => $theme->slug]) }}" class="flex flex-col items-center">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white text-center uppercase">
                                {{ $loop->parent->index * 3 + $index + 1 }} - {{ $theme->title }}
                            </h5>
                        </a>
                    </div>
                @endforeach
